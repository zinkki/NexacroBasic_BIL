...
if (!nexacro._bInitPlatform) {
	...
	__pWindow._on_default_sys_dragover = function (elem, strButton, altKey, ctrlKey, shiftKey, windowX, windowY, screenX, screenY, offsetX, offsetY) {
		var dragInfo = nexacro._cur_drag_info;
		if (dragInfo && dragInfo.targetwin == this) {
			...
			if (dragInfo._cur_elem != elem) {
				...
				if (old_comp && old_comp[0]) {
					if (old_comp[0] != comp[0]) {
						var leave_elem = (comp.length == 3) ? elem : old_comp[0]._control_element;
						if (leave_elem == elem && nexacro._use_offset && offsetX != undefined && offsetY != undefined) {
							canvasX = offsetX + old_comp[1];
							canvasY = offsetY + old_comp[2];
						}
						else {
							elem_pos = nexacro._getElementXYInWindow(leave_elem.handle);
							canvasX = windowX - elem_pos[0] + old_comp[1];
							canvasY = windowY - elem_pos[1] + old_comp[2];
						}
						...
						if (comp && comp[0]) {
							var enter_elem = (comp.length == 3) ? elem : comp[0]._control_element;
							if (enter_elem == elem && nexacro._use_offset && offsetX != undefined && offsetY != undefined) {
								canvasX = offsetX + comp[1];
								canvasY = offsetY + comp[2];
							}
							else {
								elem_pos = nexacro._getElementXYInWindow(enter_elem.handle);
			...
			dragInfo._cur_elem = elem;
			if (comp && comp[0]) {
				elem_pos, canvasX, canvasY;
				var over_elem = (comp.length == 3) ? elem : comp[0]._control_element;
				if (over_elem == elem && nexacro._use_offset && offsetX != undefined && offsetY != undefined) {
					canvasX = offsetX + comp[1];
					canvasY = offsetY + comp[2];
				}
				else {
					elem_pos = nexacro._getElementXYInWindow(over_elem.handle);
					canvasX = windowX - elem_pos[0] + comp[1];
					canvasY = windowY - elem_pos[1] + comp[2];
				}
	...
	__pWindow._on_default_sys_drop = function (elem, strButton, altKey, ctrlKey, shiftKey, windowX, windowY, screenX, screenY, offsetX, offsetY) {
		var dragInfo = nexacro._cur_drag_info;
		...
		if (elem && dragInfo && dragInfo.isDragging && dragInfo.targetwin == this) {
			...
			if (comp && comp[0]) {
				var canvasX, canvasY;
				if (elem == evt_elem && nexacro._use_offset && offsetX != undefined && offsetY != undefined) {
					canvasX = offsetX + comp[1];
					canvasY = offsetY + comp[2];
				}
				else {
					var elem_pos = nexacro._getElementXYInWindow(elem.handle, this._custom_node_id);
					canvasX = windowX - elem_pos[0] + comp[1];
					canvasY = windowY - elem_pos[1] + comp[2];
				}
		...
		if (nexacro._OS == "iOS") {
			switch (type) {
				...
				case 1:
					...
					break;
				case 2:
					var is_focused_input = elem instanceof nexacro.InputElement && (focused_comp == linkedcontrol);
					if (is_focused_input) {
						return true;
					}
