...
if (!nexacro.Grid) {
	...
	_pGridCell._isCellSuppress = function (cellinfo, datarow) {
		...
		return (disp_type != "checkboxcontrol" && disp_type != "treeitemcontrol" && disp_type != "buttoncontrol" && disp_type != "progressbarcontrol");
	};
			...
			if (!obj) {
				if (!flush) {
					if (prop == "background" || prop == "cursor" || prop == "align" || prop == "textDecoration") {
						...
						cell._control_element._flushCommand();
						flush = true;
					}
				}

				if (prop == "background") {
					obj = cell._control_element._getComputedStyleBackgroundColor(true);
				}
				else if (prop == "cursor") {
					obj = cell._control_element._getComputedStyle("cursor", true);
				}
				else if (prop == "align") {
					var align = cell._getCurrentStyleAlign(true);
	...
	_pGrid._adjustOverlayControls = function (is_create, virtual_merge) {
		if (!this.enableredraw) {
			return false;
		}
	...
	_pGrid._getSelectedCellsforRowRange = function (showrows) {
		...
		if (this.selecttype == "row") {
			...
			if ((dstoprowpos <= dscurrowidx && dscurrowidx <= dsbottomrowpos) || dscurrowidx <= this._fixed_endrow) {
			...
			for (var i = 0; i < arealen; i++) {
				...
				if ((dstoprowpos <= dsbegrowidx && dsbegrowidx <= dsbottomrowpos) || dsbegrowidx <= this._fixed_endrow) {
				...
				if ((dstoprowpos <= dsendrowidx && dsendrowidx <= dsbottomrowpos) || dsendrowidx <= this._fixed_endrow) {
			...
			if ((dstoprowpos <= dsrowidx && dsrowidx <= dsbottomrowpos) || dsrowidx <= this._fixed_endrow) {
