...
if (!nexacro.ExcelExportObject) {
	...
	_pExportItem._getFitFontValue = function (fontval) {
		...
		if (fontval) {
			...
			if (unit == "px") {
				size = nexacro._convertPxToPt(size);
			}
		}
				...
				for (j = 0; j < bodycntcell; j++) {
					...
					if (cell._subcells.length) {
						...
						for (var i = 0; i < subL; i++) {
							...
							val = subCell[i]._getDisplayText(k);
							if (exportObj._orgval_type) {
			...
			if (is_end) {
				...
				if (is_finish) {
					...
					if (exportObj._filedownload) {
						...
						if (nexacro._Browser == "Runtime" && exportfilepath_ != null && exportfilepath_ != "") {
							nexacro._download(excelURL, exportObj._hidden_frame_handle, exportfilename_, exportfilepath_, exportObj.filefilter, exportObj.filefilterindex);
						}
						else {
