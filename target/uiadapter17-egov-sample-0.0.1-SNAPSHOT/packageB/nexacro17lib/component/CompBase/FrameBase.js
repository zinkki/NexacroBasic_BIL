...
if (!nexacro.Frame) {
	...
	_pChildFrame.showModal = function (str_id, _parent_frame, arr_arg, opener, callback, is_async) {
		...
		if (_win && (_win._wheelZoom != undefined) && (_win._wheelZoom != 100)) {
			wheelZoomScale = _win._wheelZoom / 100.0;
		}
		if (_win) {
		...
		if (this.left == null) {
			if (!(isNaN(this.width) || isNaN(this.right))) {
				if (_win) {
					this.left = _win.clientWidth / wheelZoomScale - this.width - this.right;
				}
			}
		}
		if (this.top == null) {
			if (!(isNaN(this.height) || isNaN(this.bottom))) {
				if (_win) {
					this.top = _win.clientHeight / wheelZoomScale - this.height - this.bottom;
				}
			}
		}
		if (this.width == null) {
			if (!(isNaN(this.left) || isNaN(this.right))) {
				if (_win) {
					this.width = _win.clientWidth / wheelZoomScale - this.left - this.right;
				}
			}
		}
		if (this.height == null) {
			if (!(isNaN(this.top) || isNaN(this.bottom))) {
				if (_win) {
					this.height = _win.clientHeight / wheelZoomScale - this.top - this.bottom;
				}
			}
		}
		...
		if (wheelZoomScale != 1.0) {
			if (this.form._is_loaded) {
				...
				if (frameElem) {
					frameElem.setElementZoom(_win._wheelZoom);
